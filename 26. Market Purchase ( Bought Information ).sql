...
SELECT  CAST(dbo.ToBdt(tss.CreatedOn) AS DATE) PurchaseDate,e.BadgeId,e.FullName,de.DesignationName,
t.ProductVariantID ID, pv.Name Product,te.WarehouseID,w.Name WarehouseName,pv.Mrp AS MRP,
...
WHERE tss.CreatedOn >= '2022-03-05 00:00 +6:00'
AND tss.CreatedOn < '2022-03-06 00:00 +6:00'
AND FromState IN ( 262144, 536870912 ) 
AND ToState IN ( 65536,16777216,268435456 ) 
AND pv.deleted = 0 
AND t.CostPrice IS NOT NULL
GROUP BY e.BadgeId,e.FullName,de.DesignationName,t.ProductVariantID,pv.Name,pv.Mrp,te.WarehouseID,w.Name,CAST(dbo.ToBdt(tss.CreatedOn) AS DATE),t.MarketPurchaseInvoiceId
